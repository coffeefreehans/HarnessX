# HarnessX plugin services

English | [中文](plugin-services.zh.md)

This document is the supported Host-side integration contract for plugin authors. It covers the public `desktopProfiles` and `desktopPnpm` Cordis services exported by HarnessX 2.x in both compatibility and advanced presentation modes. It does not grant third-party access to raw Electron APIs, the renderer, or launcher bootstrap state.
...
The renderer receives ordinary Web Client modules over the existing loopback carrier. It cannot read these Host services directly, and HarnessX adds no preload or Electron IPC bridge for them. A plugin with browser UI continues to use normal DSH Host routes, RPC, client metadata, services, and slots.
...
import type {
  ...
  DesktopProfiles,
} from 'harnessx-desktop/profile-service'
import type {
  DesktopPnpm,
  DesktopPnpmHandle,
  DesktopPnpmOutcome,
} from 'harnessx-desktop/pnpm'
```

`harnessx-desktop/profiles` is the Desktop-owned tray consumer, not the profile service contract. Do not import it for service types.
...
| --- | --- | --- |
| `desktopProfiles` | Generation-scoped Host service. | Public and supported through `harnessx-desktop/profile-service`. |
| `desktopPnpm` | Generation-scoped Host service. | Public and supported through `harnessx-desktop/pnpm`. |
...
A plugin that only makes sense inside HarnessX can declare both services as required dependencies. Cordis keeps the plugin pending until both providers are available and unloads its effects if a required service disappears.

```ts
import type { Context } from '@deepseek-ai/cordis'
import type {} from 'harnessx-desktop/profile-service'
import type { DesktopPnpmHandle } from 'harnessx-desktop/pnpm'
...
import type { Context } from '@deepseek-ai/cordis'
import type {} from 'harnessx-desktop/profile-service'
import type {} from 'harnessx-desktop/pnpm'
...
Type-only imports are erased from JavaScript. A cross-environment package can keep `harnessx-desktop` as a development dependency for compilation, or as an optional peer if it publishes declarations that expose these types; it does not need a runtime import merely to probe the services.
...
HarnessX therefore does not preinstall or depend on that version. A compatible future release must:
