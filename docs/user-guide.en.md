...
Download the macOS or Windows installer from the product download page. HarnessX includes Electron, Node, and its pinned DSH dependencies, so normal users do not need to install Node.js or pnpm separately.
...
In the terminal opened from the HarnessX tray, bare `dsh` and plugin commands without `--profile` default to the active profile:
...
An explicit `--profile <name>` always wins. Restart HarnessX after plugin changes so the new bundle enters the Loader composition.
...
Packaged macOS and Windows applications check `https://api.github.com/repos/coffeefreehans/HarnessX/releases/latest` in the background. Startup is not blocked; network errors, non-200 responses, invalid versions, and a server version that is not newer remain silent in the background.
...
For the complete native lifecycle, packaging, and platform limits, see [`docs/package-reference.en.md`](package-reference.en.md).
