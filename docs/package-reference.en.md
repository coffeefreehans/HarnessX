...
`harnessx-desktop` runs DSH in Electron while remaining part of the ordinary Cordis composition. The installed application is named **HarnessX**. The package provides the `harnessx-desktop` executable and the `dsh-desktop` alias; the registered npm package name is the reliable `npx` entry.
...
Plugin authors should use the supported contract imports, lifecycle rules, and adaptation patterns in the [Desktop plugin service architecture](plugin-services.md).
...
HarnessX is the Yarn package at the repository root. The sibling `deepseek-harness/` checkout remains an independent upstream pnpm project and is not part of the root Yarn dependency graph. Install and verify HarnessX from the repository root:
...
- Adding or removing a profile bundle requires restarting HarnessX; the launcher does not watch profile manifests. Selecting another profile from the tray performs that restart automatically.
...
- This project currently pins the published DSH `0.1.0-rc.7` family, while the sibling `deepseek-harness/` source checkout predates that release. Tests therefore validate the published package interfaces rather than unpublished upstream sources.
