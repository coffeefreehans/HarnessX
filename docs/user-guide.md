...
从产品下载入口获取 macOS 或 Windows 安装包。安装后的 HarnessX 自带运行所需的 Electron、Node 和 DSH 依赖，普通用户不需要另行安装 Node.js 或 pnpm。
...
在 HarnessX 托盘打开的终端中，裸 `dsh` 和不带 `--profile` 的 plugin 命令默认使用当前激活 profile：
...
显式 `--profile <name>` 始终优先。插件变更后需要重启 HarnessX，才能让新的 bundle 进入 Loader 组合。
...
打包后的 macOS/Windows 应用会在后台检查 `https://api.github.com/repos/coffeefreehans/HarnessX/releases/latest`。后台检查不阻塞启动；网络错误、非 200、非法版本或服务端版本不新时保持静默。
...
更完整的原生生命周期、打包和平台限制见 [`docs/package-reference.md`](package-reference.md)。
