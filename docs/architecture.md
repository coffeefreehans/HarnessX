...
HarnessX 是一个薄的 Electron 宿主。它在 Electron main 进程中启动官方 DSH Host，Host 再通过 loopback HTTP/WebSocket 提供普通 Web UI。Desktop 没有另造一条 renderer IPC 插件系统，也不把 Electron API暴露给页面。
...
Launcher 私有的 `desktopRuntime`、`desktopPnpmBootstrap`、Electron executable、Node helper 和 ABI 环境不是第三方 API。公开 contract 只有 `harnessx-desktop/profile-service` 与 `harnessx-desktop/pnpm`。
...
根包使用 Yarn；纳入仓库的 `deepseek-harness/` 上游源码快照保持自己的 pnpm workspace。桌面代码、测试、打包配置和发布脚本直接位于仓库根目录，不修改上游源码快照。

## 维护者深入阅读

- [Desktop service contract](plugin-services.md)
- [Package README](package-reference.md)
