...
HarnessX does not reimplement Harness. It puts the same runtime into an application that is easier to start and manage on a desktop operating system:
...
- Users installing or building DSH plugins: read [plugin development](plugin-development.en.md), then the [Desktop service contract](plugin-services.md).
- Maintainers working on startup, profiles, or packaging: read the [architecture](architecture.en.md) and the package [README](package-reference.en.md).
