# HarnessX Plugin Development

## Understand the two plugin layers

A normal DSH plugin can provide Host services, commands, routes, bundles, or a Web Client. It should depend on upstream DSH contracts whenever possible so the same package can work in the CLI, an ordinary Web profile, and HarnessX.
...
The complete types, lifecycle, and failure semantics are in [`docs/plugin-services.md`](plugin-services.md). This page focuses on selection and the minimum safe patterns.
...
Read the [architecture](architecture.en.md) next, then use the package-level [service contract](plugin-services.md) as the API reference.
