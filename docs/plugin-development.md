...
一个普通 DSH 插件可以提供 Host service、命令、路由、bundle 或 Web Client。它应该尽量只依赖官方 DSH contract，因此可以在命令行、普通 Web profile 和 HarnessX 中复用。
...
完整类型、生命周期和失败语义见 [`docs/plugin-services.md`](plugin-services.md)。下面只给出选择方式和最小原则。
...
开发者可以先阅读 [架构说明](architecture.md)，再使用包级 [service contract](plugin-services.md)。
