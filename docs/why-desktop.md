...
HarnessX 的目标不是重新实现 Harness，而是把同一个运行时放进一个容易启动、容易管理、符合操作系统习惯的应用里：
...
- 想安装或开发 DSH 插件的用户：先读[插件开发](plugin-development.md)，再看 [Desktop service contract](plugin-services.md)。
- 想理解启动、profile 和打包边界的维护者：阅读[架构说明](architecture.md)和包级 [README](package-reference.md)。
