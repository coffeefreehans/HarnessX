/** HarnessX executable: minimal Electron bootstrap around the Host Cordis root. */
...
const PRODUCT_NAME = 'DeepSeek HarnessX'
...
async function start(): Promise<void> {
  ...
  runtime = new ElectronDesktopRuntime(async () => {
    if (shutdown === undefined) {
      throw new Error('harnessx-desktop: shutdown coordinator is not ready')
    }
  ...
  try {
    ...
    const ctx = await boot(
      ...
      async (hostCtx) => {
        hostCtx.effect(
          () => releasePnpmRuntime,
          'harnessx-desktop: packaged pnpm runtime PATH',
        )
        current = hostCtx
        hostCtx.effect(
          () => releasePackageResolver,
          'harnessx-desktop: profile package resolution',
        )
