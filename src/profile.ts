...
export const DESKTOP_PACKAGE_NAME = 'harnessx-desktop'
...
const DESKTOP_WINDOWS_PWSH_SANDBOX_PACKAGE = 'harnessx-desktop/windows-pwsh-sandbox'
...
/** Return only the third-party bundles owned by the user or plugin command. */
export function desktopThirdPartyBundles(current: readonly string[]): string[] {
  return current.filter(name => !REQUIRED_BUNDLE_SET.has(name) && name !== DESKTOP_PACKAGE_NAME)
}
...
export function desktopBundleList(current: readonly string[]): string[] {
  return [...REQUIRED_BUNDLES, ...desktopThirdPartyBundles(current)]
}
...
export function prepareDesktopProfile(
  ...
  const directoryPicker = rows.get('directory-picker')
    ...
    patches.push(
      ...
      },
      {
        insert: [
          ...
          {
            id: 'desktop-directory-picker-surface',
