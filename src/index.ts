/** HarnessX Host plugin: owns the selected native shell generation. */
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (appExit === undefined) {
    throw new Error('harnessx-desktop: the launcher did not provide ctx.appExit')
  }
  if (ctx.webServer.host !== '127.0.0.1') {
    throw new Error('harnessx-desktop: desktop shell requires a loopback Web server')
  }
    ...
    const stopWatching = settings.watch((next) => {
      ...
      pending ??= setImmediate(() => {
        ...
        void ctx.desktopRuntime.requestRestart().catch((cause: unknown) => {
          ctx.logger.error('harnessx-desktop: failed to restart after mode change')
  ...
  }, 'harnessx-desktop: restart after mode change')
  ...
  ctx.effect(
    () => ctx.desktopRuntime.schedule({
      ...
      readThemeSource: () => {
        ...
        if (theme === undefined) {
          throw new Error('harnessx-desktop: advanced shell requires the ui-theme settings namespace')
        }
    ...
    'harnessx-desktop: native shell generation',
  )
}
