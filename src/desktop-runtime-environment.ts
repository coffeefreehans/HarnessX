...
function assertScriptValue(label: string, value: string): void {
  if (value.length === 0) {
    throw new Error(`harnessx-desktop: pnpm runtime ${label} must not be empty`)
  }
  if (/[\0\r\n]/u.test(value)) {
    throw new Error(`harnessx-desktop: pnpm runtime ${label} must not contain NUL or newlines`)
  }
}
...
function quoteBatchWord(value: string): string {
  if (/["\r\n]/u.test(value)) {
    throw new Error('harnessx-desktop: pnpm runtime Windows arguments must not contain quotes or newlines')
  }
  return `"${value.replaceAll('%', '%%')}"`
}
...
function escapeBatchSetValue(value: string): string {
  if (/["\r\n]/u.test(value)) {
    throw new Error('harnessx-desktop: pnpm runtime Windows environment values must not contain quotes or newlines')
  }
  return value.replaceAll('%', '%%')
}
...
function preparePrivateDirectory(directory: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error(`harnessx-desktop: pnpm runtime path is not a private directory: ${directory}`)
  }
...
function assertOwnedDirectoryEntries(directory: string, allowed: readonly string[]): void {
  ...
  if (unexpected.length > 0) {
    throw new Error(
      `harnessx-desktop: pnpm runtime directory contains unexpected entries: ${unexpected.join(', ')}`,
    )
  }
}
...
function removeStaleTemporaryFiles(directory: string, targetName: string): void {
  ...
  for (const entry of readdirSync(directory)) {
    ...
    if (!stat.isFile() && !stat.isSymbolicLink()) {
      throw new Error(`harnessx-desktop: pnpm runtime stale temporary path is not a file: ${filename}`)
    }
    unlinkSync(filename)
  }
}
...
function replacePrivateFile(filename: string, contents: string, mode: number): void {
  ...
  if (existing !== undefined && (!existing.isFile() || existing.isSymbolicLink())) {
    throw new Error(`harnessx-desktop: pnpm runtime file is not a regular file: ${filename}`)
  }
...
export function installDesktopPnpmRuntime(options: DesktopPnpmRuntimeOptions): DesktopPnpmRuntimeInstallation {
  if (options.platform !== 'darwin' && options.platform !== 'linux' && options.platform !== 'win32') {
    throw new Error(`harnessx-desktop: pnpm runtime is unsupported on ${options.platform}`)
  }
