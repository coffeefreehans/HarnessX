/** Headless-safe npm launcher for the HarnessX Electron executable. */
...
export const DESKTOP_CLI_HELP = `Usage: harnessx-desktop [options]

Launch DeepSeek HarnessX with the selected Web-capable profile.
...
export async function runDesktopCli(argv: readonly string[]): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(`harnessx-desktop: ${cause instanceof Error ? cause.message : String(cause)}\n`)
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  void runDesktopCli(process.argv.slice(2)).then(
    ...
    cause => {
      process.stderr.write(`harnessx-desktop: ${cause instanceof Error ? cause.stack ?? cause.message : String(cause)}\n`)
      process.exitCode = 1
    },
  )
}
