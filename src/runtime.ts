...
import type { UpdateCheckResult, UpdateReleaseInfo, UpdateRequest } from './update-checker.ts'

/** Electron platforms supported by the HarnessX native adapter. */
export type DesktopPlatform = 'darwin' | 'win32' | 'linux'

/** Electron CPU architectures supported by HarnessX release assets. */
...
export interface DesktopUpdateAdapter {
  ...
  /** CPU architecture selecting a GitHub Release artifact. */
...
export interface DesktopRuntime {
  ...
  /** Authorize one browser request against the per-launch local API session. */
  authorizeLocalApiRequest(request: Pick<IncomingMessage, 'headers' | 'method'>): boolean
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    /** Electron adapter provided by the HarnessX launcher. */
