...
export class DesktopProfileService extends Service implements DesktopProfiles {
  ...
  constructor(ctx: Context, private readonly bootstrap: DesktopProfileServiceBootstrap) {
    ...
    ctx.effect(
      () => () => { this.disposed = true },
      'harnessx-desktop: profile service lifetime',
    )
  }
  ...
  private committedSelectionError(name: string): Error {
    return new Error(
      `harnessx-desktop: profile ${JSON.stringify(this.committedName)} is already pending; `
  ...
  private assertActive(): void {
    if (this.disposed) throw new Error('harnessx-desktop: desktopProfiles service disposed')
  }
}
