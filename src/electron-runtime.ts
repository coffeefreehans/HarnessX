...
import type { UpdateCheckResult, UpdateReleaseInfo } from './update-checker.ts'
...
export function desktopProductVersion(moduleUrl: string = import.meta.url): string {
  ...
  if (value === null || typeof value !== 'object' || typeof (value as { version?: unknown }).version !== 'string') {
    throw new Error('harnessx-desktop: package.json has no product version')
  }
...
/** Return the current CPU architecture supported by release artifacts. */
function desktopArch(): DesktopArch {
  if (process.arch === 'x64' || process.arch === 'arm64') return process.arch
  throw new Error(`harnessx-desktop: unsupported update architecture ${process.arch}`)
}

/** Native adapter used by the HarnessX launcher and owned by its Cordis shell plugin. */
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  private readonly extraMainWindows = new Set<BrowserWindow>()

  constructor(private readonly restart: () => Promise<void>) {
    if (process.platform !== 'darwin' && process.platform !== 'win32' && process.platform !== 'linux') {
      throw new Error(`harnessx-desktop: unsupported Electron platform ${process.platform}`)
    }
  ...
  schedule(spec: DesktopShellSpec): () => Promise<void> {
    if (this.scheduled !== undefined || this.mountTask !== undefined) {
      throw new Error('harnessx-desktop: a native shell generation is already registered')
    }
    const previousThemeSource = nativeTheme.themeSource
    const localApiSession = createLocalApiSession(new URL(spec.url).origin)
    this.localApiSession = localApiSession
    this.scheduled = spec
    let disposed = false
    return async () => {
      if (disposed) return
      ...
      try {
        ...
        try {
          ...
          if (this.scheduled === spec) {
            if (this.localApiSession === localApiSession) this.localApiSession = undefined
  ...
  /** @inheritdoc */
  authorizeLocalApiRequest(request: Parameters<DesktopRuntime['authorizeLocalApiRequest']>[0]): boolean {
    return authorizeLocalApiRequest(request, this.localApiSession)
  }
  ...
  mountScheduled(beforeInteractive?: () => void): Promise<void> {
    ...
    if (spec === undefined) {
      return Promise.reject(new Error('harnessx-desktop: the Cordis shell plugin did not register a window'))
    }
  ...
  async openMainWindow(): Promise<void> {
    const spec = this.scheduled
    if (spec === undefined) {
      throw new Error('harnessx-desktop: no shell generation is registered for a main window')
    }
    const origin = new URL(spec.url).origin
    const localApiSession = this.localApiSession
    if (localApiSession === undefined || localApiSession.origin !== origin) {
    ...
    this.extraMainWindows.add(window)
    const forget = (): void => { this.extraMainWindows.delete(window) }
    const close = (event: Electron.Event): void => {
      if (this.quitting) return
      // Same tray lifecycle as the primary shell window: hide, keep running.
  ...
  configureTerminal(spec: DesktopTerminalSpec): void {
    if (this.terminalSpec !== undefined) {
      throw new Error('harnessx-desktop: terminal profile is already configured')
    }
  ...
  openTerminal(): void {
    try {
      ...
      if (spec === undefined) {
        throw new Error('harnessx-desktop: terminal profile is not configured')
      }
      const electronVersion = process.versions.electron
      if (electronVersion === undefined) {
        throw new Error('harnessx-desktop: terminal requires the Electron runtime version')
      }
      openDesktopTerminal({
  ...
  private trayCommand(invoke: () => void | Promise<void>): () => void {
    return () => {
      void Promise.resolve().then(invoke).catch((cause: unknown) => {
        process.stderr.write(`harnessx-desktop: tray command failed: ${cause instanceof Error ? cause.message : String(cause)}\n`)
      })
    }
  }
  ...
  private async confirmUpdateDownload(version: string): Promise<boolean> {
    const result = await dialog.showMessageBox({
      type: 'info',
      title: 'DeepSeek HarnessX Update Available',
      message: `DeepSeek HarnessX ${version} is available.`,
  ...
  private async showManualUpdateCheckResult(result: UpdateCheckResult | null): Promise<void> {
    if (result === null) {
      await dialog.showMessageBox({
        ...
        message: 'DeepSeek HarnessX could not check for updates.',
    ...
    if (result.status === 'up-to-date') {
      await dialog.showMessageBox({
        type: 'info',
        title: 'DeepSeek HarnessX Is Up to Date',
        message: 'No newer version of DeepSeek HarnessX is available.',
    ...
    await dialog.showMessageBox({
      type: 'info',
      title: 'DeepSeek HarnessX Update Available',
      message: `DeepSeek HarnessX ${result.latestVersion} is available.`,
  ...
  private async downloadAndOpenUpdate(
    ...
    if (this.platform === 'darwin') {
      ...
      if (openError !== '') throw new Error(`harnessx-desktop: failed to open update disk image: ${openError}`)
      signal.throwIfAborted()
      await dialog.showMessageBox({
        type: 'info',
        title: 'DeepSeek HarnessX Update Downloaded',
        message: `DeepSeek HarnessX ${version} is ready to install.`,
        detail: 'The disk image has opened. Replace DeepSeek HarnessX in Applications, then reopen it.',
        buttons: ['OK'],
        defaultId: 0,
        noLink: true,
      })
    ...
    const result = await dialog.showMessageBox({
      ...
      title: 'DeepSeek HarnessX Update Downloaded',
      message: `DeepSeek HarnessX ${version} is ready to install.`,
      detail: 'Restart DeepSeek HarnessX and run the installer now?',
    ...
    const spec = this.scheduled
    if (spec === undefined) throw new Error('harnessx-desktop: no active shell can exit for update installation')
    signal.throwIfAborted()
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once('spawn', () => {
        ...
        child.once('error', cause => {
          process.stderr.write(`harnessx-desktop: update installer failed after launch: ${cause.message}\n`)
        })
        child.unref()
        resolve()
      })
  ...
  private reportTerminalLaunchError(cause: unknown): void {
    ...
    process.stderr.write(`harnessx-desktop: failed to open terminal: ${error.message}\n`)
    try {
      dialog.showErrorBox('Unable to Open DSH Terminal', error.message)
    } catch (dialogCause) {
      process.stderr.write(`harnessx-desktop: failed to show terminal error: ${dialogCause instanceof Error ? dialogCause.message : String(dialogCause)}\n`)
    }
  }
  ...
  private async mount(
      ...
      try {
        ...
        if (target.protocol === 'https:' || target.protocol === 'http:' || target.protocol === 'mailto:') {
          void shell.openExternal(target.href).catch((cause: unknown) => {
            process.stderr.write(`harnessx-desktop: failed to open external link: ${cause instanceof Error ? cause.message : String(cause)}\n`)
          })
        }
      } catch {
        // A malformed target is rejected with the same deny result.
    ...
    if (tray === undefined) {
      throw new Error('harnessx-desktop: native tray did not mount')
    }
