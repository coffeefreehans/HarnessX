/** Isolated command-line environment launched from the HarnessX tray. */
...
/** Platforms with a native terminal launch contract owned by HarnessX. */
...
function assertScriptValue(label: string, value: string): void {
  if (value.length === 0) throw new Error(`harnessx-desktop: terminal ${label} must not be empty`)
  if (/[\0\r\n]/.test(value)) {
    throw new Error(`harnessx-desktop: terminal ${label} must not contain NUL or newlines`)
  }
}
...
function escapeBatchText(value: string): string {
  if (/\r|\n/.test(value)) {
    throw new Error('harnessx-desktop: terminal Windows welcome text must not contain newlines')
  }
  return value
    .replaceAll('^', '^^')
    .replaceAll('%', '%%')
...
function prepareStateDirectory(stateDir: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error(`harnessx-desktop: terminal state path is not a private directory: ${stateDir}`)
  }
...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`HarnessX ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart HarnessX after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("HarnessX {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart HarnessX after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(HarnessX !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart HarnessX after plugin changes.')}`,
...
function prepareDesktopTerminalFiles(options: DesktopTerminalOptions): DesktopTerminalFiles {
  if (options.platform !== 'darwin' && options.platform !== 'win32') {
    throw new Error(`harnessx-desktop: terminal is unsupported on ${options.platform}`)
  }
...
function resolveWindowsShell(
  ...
  throw new Error('harnessx-desktop: terminal requires pwsh.exe, powershell.exe, or cmd.exe on Windows')
}
...
function windowsShellArgv(
  ...
  if (shell.kind === 'cmd') {
    ...
    if (welcome === undefined) {
      throw new Error('harnessx-desktop: terminal did not generate the Windows command prompt welcome script')
    }
...
function resolveWindowsCommandProcessor(
  ...
  if (executable === undefined) {
    throw new Error('harnessx-desktop: terminal requires cmd.exe to create a visible Windows console')
  }
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "HarnessX" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
...
function reportLaunchError(options: DesktopTerminalOptions, cause: Error): void {
  if (options.onLaunchError !== undefined) {
    try {
      ...
      process.stderr.write(`harnessx-desktop: terminal error reporter failed: ${detail}\n`)
      return
    }
  }
  process.stderr.write(`harnessx-desktop: failed to open terminal: ${cause.message}\n`)
}
