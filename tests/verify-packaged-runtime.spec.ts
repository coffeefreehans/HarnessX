...
import {
  ...
  REQUIRED_WINDOWS_NATIVE_ENTRIES,
...
function context(appOutDir: string, electronPlatformName: string): PackagedRuntimeContext {
  return {
    ...
    packager: { appInfo: { productFilename: 'HarnessX' } },
  }
}
...
describe('packaged desktop runtime verification', () => {
  it.each([
    [
      'darwin',
      join('/build', 'HarnessX.app', 'Contents', 'Resources', 'app.asar'),
    ],
    [
      'win32',
    ...
    expect(exists).toHaveBeenCalledTimes(
      REQUIRED_UNPACKED_RUNTIME_ENTRIES.length
        + (platform === 'win32' ? REQUIRED_WINDOWS_NATIVE_ENTRIES.length : 0),
    )
