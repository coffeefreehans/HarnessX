...
/** Stable GitHub release fixture used by native update handoff tests. */
const UPDATE_RELEASE: UpdateReleaseInfo = {
  version: '2.1.0',
  tagName: 'v2.1.0',
  releaseName: 'HarnessX 2.1.0',
  releaseNotes: 'Release notes',
  publishedAt: '2026-08-17T00:00:00Z',
  releaseUrl: 'https://github.com/coffeefreehans/HarnessX/releases/tag/v2.1.0',
  assets: [],
}
...
const spec: DesktopShellSpec = {
  ...
  productName: 'HarnessX',
  windowTitle: 'HarnessX',
...
describe('Electron compatibility runtime', () => {
  ...
  it('uses the native macOS frame, Dock icon, and template tray image', async () => {
    ...
    expect(electron.webContents.session.cookies.set).toHaveBeenCalledWith(expect.objectContaining({
      url: 'http://127.0.0.1:43120',
      name: 'harnessxLocalSession',
      httpOnly: true,
      sameSite: 'strict',
    }))
    ...
    expect(electron.browserWindows[0]?.accessibleTitle).toBe('HarnessX')
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    updater.download.mockResolvedValueOnce('/tmp/HarnessX-2.1.0-x64.dmg')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})

    await expect(runtime.updates.request('https://api.github.com/repos/coffeefreehans/HarnessX/releases/latest', { method: 'GET' }))
    ...
    await runtime.updates.downloadAndOpen('2.1.0', UPDATE_RELEASE, controller.signal)
    ...
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/HarnessX-2.1.0-x64.dmg')
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
  ...
  it('starts the downloaded Windows installer before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\HarnessX-2.1.0-x64-Setup.exe')
    const requestQuit = vi.fn()
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    runtime.schedule({ ...spec, requestQuit })

    const pending = runtime.updates.downloadAndOpen('2.1.0', UPDATE_RELEASE, new AbortController().signal)
    await vi.waitFor(() => { expect(childProcess.spawn).toHaveBeenCalledOnce() })
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\HarnessX-2.1.0-x64-Setup.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
  ...
  it('keeps a downloaded Windows installer idle when installation is deferred', async () => {
    ...
    await runtime.updates.downloadAndOpen('2.1.0', UPDATE_RELEASE, new AbortController().signal)
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    await expect(runtime.updates.downloadAndOpen('2.1.0', UPDATE_RELEASE, new AbortController().signal))
  ...
  it('does not show macOS completion after the update generation is cancelled', async () => {
    ...
    const pending = runtime.updates.downloadAndOpen('2.1.0', UPDATE_RELEASE, controller.signal)
