...
describe('published package surface', () => {
  it('registers both npm launcher names', () => {
    expect(manifest.name).toBe('harnessx-desktop')
    expect(manifest.bin).toEqual({
      harnessx: 'lib/bin.js',
      'harnessx-desktop': 'lib/bin.js',
    })
  })
  ...
  it('installs the private pnpm PATH after the launch snapshot and before profile boot', () => {
    ...
    expect(main).toContain("'harnessx-desktop: packaged pnpm runtime PATH'")
    expect(main).toContain('disposePnpmRuntime?.()')
  })

  it('fixes the installed application identity', () => {
    expect(manifest.version).toBe(workspaceManifest.version)
    expect(manifest.build?.productName).toBe('DeepSeek HarnessX')
    expect(manifest.build?.appId).toBe('io.github.coffeefreehans.harnessx')
    ...
    expect(manifest.build?.mac?.artifactName).toBe('HarnessX-${version}-${arch}.${ext}')
  ...
  it('separates unsigned smoke packaging from the signed macOS release', () => {
    ...
    expect(manifest.scripts?.build).toContain('node scripts/generate-win-app-icon.mjs')
    ...
    expect(workspaceManifest.scripts?.['dist:mac']).toBe('node scripts/release-mac.ts')
    expect(workspaceManifest.scripts?.['dist:win']).toBe('node scripts/package-win.ts')
