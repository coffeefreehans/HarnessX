...
function macOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'darwin',
    appExecutable: "/Applications/DSH O'Brien.app/Contents/MacOS/HarnessX",
...
function windowsOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'win32',
    appExecutable: 'C:\\Program Files\\DSH 100% Desktop\\HarnessX.exe',
    dshBootstrapPath: 'C:\\Program Files\\HarnessX\\resources\\app.asar\\lib\\dsh-terminal-bootstrap.js',
    pnpmBinPath: 'C:\\Program Files\\HarnessX\\resources\\app.asar\\node_modules\\pnpm\\bin\\pnpm.mjs',
    electronVersion: '43.4.0',
    profileName: 'desktop',
    productVersion: '2.0.0',
...
describe('desktop terminal environment', () => {
  ...
  it('generates private macOS shims and opens one quoted welcome command', () => {
    ...
    expect(dshShim).toContain("'/Applications/DSH O'\"'\"'Brien.app/Contents/MacOS/HarnessX'")
    ...
    expect(nodeShim).toBe([
      '#!/bin/sh',
      `ELECTRON_RUN_AS_NODE=1 exec '/Applications/DSH O'"'"'Brien.app/Contents/MacOS/HarnessX' "$@"`,
      '',
    ].join('\n'))
    ...
    expect(welcome).toContain('HarnessX 2.0.0 terminal')
    ...
    expect(welcome).toContain('Restart HarnessX after plugin changes.')
  ...
  it('generates Windows batch shims and opens PowerShell through a visible-console broker', () => {
    ...
    expect(welcome).toContain('"HarnessX {0} terminal" -f $env:DSH_DESKTOP_PRODUCT_VERSION')
    ...
    expect(launcher).toContain('start "HarnessX" /D "!DSH_DESKTOP_PROFILE_DIRECTORY!"')
  ...
  it('keeps Windows scripts ASCII and passes localized paths through the child environment', () => {
    ...
    options.appExecutable = 'C:\\程序\\HarnessX.exe'
    ...
    expect(harness.calls[0]?.options.env).toEqual(expect.objectContaining({
      ...
      DSH_DESKTOP_APP_EXECUTABLE: 'C:\\程序\\HarnessX.exe',
