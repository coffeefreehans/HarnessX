...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode before loading the fixed packaged CLI entry', async () => {
    ...
    const argv = ['/Applications/HarnessX', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/HarnessX',
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    const packageRoot = resolve('HarnessX.app', 'Contents', 'Resources')
    const moduleUrl = pathToFileURL(join(packageRoot, 'app.asar', 'lib', 'desktop-cli.js')).href
    expect(unpackedAsarPath('/Applications/HarnessX.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/HarnessX.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\HarnessX\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\HarnessX\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/HarnessX.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/HarnessX.app/Contents/Resources/app.asar.unpacked/package.json')
