...
/** Architecture-specific native entries required when Windows packages skip source rebuilds. */
export const REQUIRED_WINDOWS_NATIVE_ENTRIES = [
  ...
  'node_modules/node-addon-require-builtin-win32-arm64-msvc/prebuilt/win32-arm64-msvc-napi-v9.node',
...
export const REQUIRED_UNPACKED_PACKAGE_SPECIFIERS = [
  'harnessx-desktop',
  'harnessx-desktop/profile',
  'harnessx-desktop/client',
  'harnessx-desktop/terminal',
  'harnessx-desktop/pnpm',
  'harnessx-desktop/profile-service',
  'harnessx-desktop/profiles',
  'harnessx-desktop/updates',
  'harnessx-desktop/windows-pwsh-sandbox',
  'harnessx-desktop/package.json',
...
export function resolvePackagedAsarPath(context: PackagedRuntimeContext): string {
  ...
  throw new Error(
    `harnessx-desktop: unsupported Electron afterPack platform ${JSON.stringify(context.electronPlatformName)}`,
  )
}
...
export function verifyPackagedAsar(
  ...
  try {
    ...
    throw new Error(
      `harnessx-desktop: failed to inspect packaged runtime at ${archivePath}`,
      { cause },
    )
  }
  ...
  if (missing.length > 0) {
    throw new Error(
      `harnessx-desktop: packaged runtime at ${archivePath} is missing required ASAR entries: ${missing.join(', ')}`,
    )
  }
}
...
export function verifyUnpackedPackageResolution(
  ...
  for (const specifier of REQUIRED_UNPACKED_PACKAGE_SPECIFIERS) {
    ...
    try {
      ...
      throw new Error(
        `harnessx-desktop: packaged runtime at ${unpackedRoot} cannot resolve required package export ${specifier}`,
        { cause },
      )
    }
    ...
    if (
      ...
      throw new Error(
        `harnessx-desktop: required package export ${specifier} resolved outside ${unpackedRoot}: ${resolvedPath}`,
      )
    }
  }
}
...
export function verifyPackagedRuntime(
  ...
  const requiredPhysicalEntries = context.electronPlatformName === 'win32'
    ? [...REQUIRED_UNPACKED_RUNTIME_ENTRIES, ...REQUIRED_WINDOWS_NATIVE_ENTRIES]
    : REQUIRED_UNPACKED_RUNTIME_ENTRIES
  const missing = requiredPhysicalEntries.filter(entry => !exists(join(unpackedRoot, entry)))
  if (missing.length > 0) {
    throw new Error(
      `harnessx-desktop: packaged runtime at ${unpackedRoot} is missing required physical entries: ${missing.join(', ')}`,
    )
  }
